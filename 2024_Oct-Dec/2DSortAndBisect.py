...
class Solution:
    def maximumBeauty(self, items: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for item in items:
            ...
            dict1[item[0]]=maximum

        result=[]
        for query in queries:
            index=bisect.bisect_left(items, query, key=lambda x:x[0])
            if index==0 and items[index][0]!=query:
                val=0
            elif (index==length) or (items[index][0]!=query):
                val=dict1[items[index-1][0]]
            else:
                val=dict1[items[index][0]]
            result.append(val)
        return result
sol=Solution()
